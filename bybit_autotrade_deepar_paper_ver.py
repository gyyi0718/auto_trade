...
def try_enter(symbol: str, side_override: Optional[str] = None, size_pct: Optional[float] = None):
    ...
    b, a, mid = get_quote(symbol)
    if mid <= 0:
        return

    # --- EV 필터(옵션, USE_EV=1일 때만) ---
    if USE_EV:
        try:
    ...
    print(f"[TP/SL] lev={LEVERAGE} entry={entry:.6f} "
...
def monitor_loop(poll_sec: float = 1.0):
    """포지션 변화 감지 + 신규 포지션 TP/SL/분할익절 세팅 + 보유시간 관리"""
    def _force_bps(x) -> float:
        x = float(x)
        if x <= 0: return 0.0
        if x < 1.0:  return x * 100.0
