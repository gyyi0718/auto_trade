# bybit_test_tcn_daily.py  (patched)
# -*- coding: utf-8 -*-
"""
...
ENTRY_TOL_BPS    = env_float("ENTRY_TOL_BPS", 5.0)   # ← 신규: 근접 진입 허용 오차(bps)
...
def run_day_once(symbols: List[str], plan: Dict[str,dict]):
    ...
    while True:
        ...
        for s in symbols:
            ...
            # 진입 조건: (기존) 터치 OR (신규) ±ENTRY_TOL_BPS 근접
            trig = False
            if side=="Buy":
                tol_ok, dev = within_tol(mid, limit, ENTRY_TOL_BPS)
                trig = (bid <= limit) or (mid <= limit) or tol_ok
                why = f"touch" if (bid<=limit or mid<=limit) else (f"proximity {dev:.2f}bps" if tol_ok else "")
                ...
                why = f"touch" if (ask>=limit or mid>=limit) else (f"proximity {dev:.2f}bps" if tol_ok else "")
                if s not in brok.pos and trig:
