...
# ===== API 클래스 =====
class BinanceAPI:
    """Binance Futures API 클라이언트 (공통)"""

    def __init__(self):
        # Testnet과 Mainnet URL이 다름
        if USE_TESTNET:
    ...
    def get_ticker(self, symbol: str) -> dict:
        ...
        try:
            ...
            # Binance는 성공 시 {"symbol": "BTCUSDT", "price": "50000.00"} 형태
            # 에러 시 {"code": -1000, "msg": "..."} 형태
            if "code" in data and data["code"] != 0:
                print(f"⚠️  Ticker API Error ({symbol}): {data.get('msg', 'Unknown')}")
                return {}

            # 성공 시 price를 lastPrice로 매핑하여 호환성 유지
            if "price" in data:
    ...
    def get_klines(self, symbol: str, interval: str = "5m", limit: int = 200) -> pd.DataFrame:
        ...
        try:
            ...
            # Binance는 에러 시 dict, 성공 시 list
            if isinstance(data, dict) and "code" in data:
                print(f"⚠️  Klines API Error ({symbol}): code={data.get('code')}, msg={data.get('msg', 'Unknown')}")
                return pd.DataFrame()
            ...
            # Binance 응답: [timestamp, open, high, low, close, volume, close_time, quote_volume, trades, ...]
