# -*- coding: utf-8 -*-
# PAPER 전용 (확장판, HEDGE 지원): DeepAR + TCN 듀얼 합의
# - 헤지 모드: 심볼별 Buy/Sell 포지션을 독립 관리(동시 보유)
# - 정책:
#   HEDGE_MODE=1          → 헤지 활성
#   HEDGE_POLICY=signals  → 각 방향 신호 따로 보고 들어감(기본)
#   HEDGE_POLICY=both     → 신호와 무관하게 양방향 동시 진입 시도
#   ENTRY_EQUITY_PCT_BOTH → both일 때 각 방향에 배분할 비율(기본 ENTRY_EQUITY_PCT/2)
...
# === Hedge ===
HEDGE_MODE = str(os.getenv("HEDGE_MODE","1")).lower() in ("1","y","yes","true")
HEDGE_POLICY = os.getenv("HEDGE_POLICY","signals").lower()  # signals|both
ENTRY_EQUITY_PCT_BOTH = float(os.getenv("ENTRY_EQUITY_PCT_BOTH", str(max(1e-9, ENTRY_EQUITY_PCT/2.0))))
...
ENTRY_HISTORY: Dict[Tuple[str,str], collections.deque] = {(s,sd): collections.deque(maxlen=max(1,N_CONSEC_SIGNALS)) for s in SYMBOLS for sd in ("Buy","Sell")}
...
def _ensure_csv():
    ...
    if not os.path.exists(EQUITY_MTM_CSV):
        with open(EQUITY_MTM_CSV,"w",newline="",encoding="utf-8") as f: csv.writer(f).writerow(EQUITY_FIELDS)
...
def get_equity_mtm() -> float:
    ...
    for (sym,side), p in BROKER.positions().items():
        ...
        if mark > 0:
            upnl += unrealized_pnl_usd(side, p["qty"], p["entry"], mark)
    return base + upnl
...
def deepar_time_to_target_minutes(symbol: str, side: str, target_bps: float) -> Optional[int]:
    ...
    try:
        ...
        step_mu = np.array(pred[0]).reshape(-1)
    except Exception:
        try:
    ...
    if path[min(hit_idx, len(path)-1)] < thr: return None
...
def _build_tcn_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ema_win=60; alpha=2/(ema_win+1); ema=np.zeros_like(c, dtype=np.float64); ema[0]=c[0]
...
def tcn_direction(symbol: str) -> Tuple[Optional[str], float, float]:
    if TCN_MODEL is None: return (None, 0.0, _tcn_thr())
    df = get_recent_1m(symbol, minutes=TCN_SEQ_LEN + 10)
    if df is None or len(df) < (TCN_SEQ_LEN+1): return (None, 0.0, _tcn_thr())
...
def ma_side(symbol: str, minutes: int = 180) -> Optional[str]:
    ...
    c = df["close"].to_numpy()
    ma20 = np.mean(c[-20:]); ma60 = np.mean(c[-60:])
...
def deepar_direction(symbol: str):
    ...
    if base is None: base = ma_side(symbol)

    if ENTRY_MODE == "model": side = base
...
@torch.no_grad()
def dual_ok(symbol: str, want_side: str) -> bool:
    d_side, _ = deepar_direction(symbol)
    t_side, mu_t, thr_t = tcn_direction(symbol)
    return (d_side == want_side) and (t_side == want_side) and (abs(mu_t) >= thr_t)

@torch.no_grad()
def time_to_target_minutes(symbol: str, side: str, target_bps: float) -> Optional[int]:
    # TCN 추정 + DeepAR 폴백
    t_side, mu_t, _thr = tcn_direction(symbol)
    if t_side == side and mu_t is not None:
        step = abs(float(mu_t))
        if step > 1e-8:
            thr = _log_thr_from_bps(target_bps)
            need = int(math.ceil(thr / step))
            return max(1, need)
    return deepar_time_to_target_minutes(symbol, side, target_bps)
...
def calc_timeout_minutes(symbol: str, side: Optional[str] = None) -> int:
    ...
    if TIMEOUT_MODE == "model":
        ...
        mins = []
        use_side = side or "Buy"
        if TIMEOUT_SOURCE in ("both","tcn"):
            m2 = time_to_target_minutes(symbol, use_side, target_bps)
        ...
        if mins:
            if TIMEOUT_AGG == "avg": chosen = int(math.ceil(sum(mins)/len(mins)))
            elif TIMEOUT_AGG == "max": chosen = max(mins)
            else: chosen = min(mins)
            chosen = int(min(max(chosen, TIMEOUT_MIN), TIMEOUT_MAX))
            if VERBOSE: print(f"[TIMEOUT][{symbol}] side={use_side} -> {chosen}m (model)")
            return chosen
...
_ADAPTERS: Dict[str, OnlineAdapter] = collections.defaultdict(lambda: OnlineAdapter(ONLINE_LR, ONLINE_DECAY, ONLINE_MAXBUF))
...
class PaperBroker:
    """
    positions: dict[(symbol, side)] = {side, entry, qty, opened}
    """
    def __init__(self):
        self.pos: Dict[Tuple[str,str],dict] = {}
        self.qty_step = 0.001
        self.min_qty = 0.001
    def has_pos(self, symbol: str, side: str) -> bool:
        return (symbol, side) in self.pos
    def try_open(self, symbol: str, side: str, mid: float, ts: int, equity_pct: float):
        eq = get_wallet_equity()
        notional = eq * max(equity_pct, 0.0) * LEVERAGE
        qty = _round_down(notional / max(mid,1e-9), self.qty_step)
        if qty < self.min_qty: return False
        self.pos[(symbol, side)] = {"side": side, "entry": mid, "qty": qty, "opened": ts}
        return True
    def try_close(self, symbol: str, side: str, ts: int) -> Optional[float]:
        key = (symbol, side)
        if key not in self.pos: return None
        _,_,mark = get_quote(symbol)
        p = self.pos[key]; entry=p["entry"]; qty=p["qty"]
...
# ===== Signals → desired sides =====
def desired_sides(symbol: str) -> List[str]:
    if not HEDGE_MODE:
        # 기존 단일 방향
        side, _ = deepar_direction(symbol)
        return [side] if side in ("Buy","Sell") else []
    # 헤지 모드
    if HEDGE_POLICY == "both":
        return ["Buy","Sell"]
    # signals: 각 방향 독립 판정
    sides = []
    for sd in ("Buy","Sell"):
        if dual_ok(symbol, sd): sides.append(sd)
    return sides
...
def open_position(symbol: str, side: str):
    ...
    if SESSION_MAX_TRADES > 0 and SESSION_TRADES >= SESSION_MAX_TRADES:
        if VERBOSE:
            ...
            print(f"[SKIP] session cap (remain {max(remain,0)}s)")
        return

    key = (symbol, side)
    if COOLDOWN_UNTIL.get(key, 0.0) > time.time(): return
    if BROKER.has_pos(symbol, side): return
    ...
    with ENTRY_LOCK:
        if BROKER.has_pos(symbol, side): return
        eq_pct = ENTRY_EQUITY_PCT if not HEDGE_MODE else (ENTRY_EQUITY_PCT if HEDGE_POLICY=="signals" else ENTRY_EQUITY_PCT_BOTH)
        if not BROKER.try_open(symbol, side, mid, ts, equity_pct=eq_pct): return
        ...
        pos_qty = BROKER.positions()[(symbol, side)]["qty"]
        STATE[(symbol, side)] = {
        ...
        if VERBOSE:
            print(f"[OPEN] {symbol} {side} entry={mid:.4f} timeout={horizon_min}m "
                  f"abs≈{STATE[(symbol,side)]['abs_usd']:.2f}USD  [EQUITY]={get_wallet_equity():.2f}")

def try_enter(symbol: str):
    # 연속 신호 필터(방향별)
    sides = desired_sides(symbol)
    for side in sides:
        dq = ENTRY_HISTORY[(symbol, side)]
        need = max(1, N_CONSEC_SIGNALS)
        dq.append(side)
        if need > 1 and (len(dq) < need or any(x != side for x in dq)):
            if VERBOSE: print(f"[SKIP] consec fail {(symbol,side)} need={need} got={list(dq)}")
            continue
...
def close_and_log(symbol: str, side: str, reason: str):
    now = int(time.time())
    key = (symbol, side)
    if key not in BROKER.positions(): return
    p = BROKER.positions()[key]
    entry = p["entry"]; qty = p["qty"]
    _,_,mark = get_quote(symbol)
    BROKER.try_close(symbol, side, now)
    ...
    if VERBOSE: print(f"[{reason}] {symbol}/{side} gross={gross:.6f} fee={fee:.6f} eq={new_eq:.2f}")
    POSITION_DEADLINE.pop(key, None)
    STATE.pop(key, None)
...
# ===== Main =====
def main():
    print(f"[START] PAPER EXTENDED | MODE={ENTRY_MODE} TESTNET={TESTNET} HEDGE_MODE={int(HEDGE_MODE)} policy={HEDGE_POLICY}")
