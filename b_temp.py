...
def predict_minute_model(train_path, model_path):
    x_test, y_test = get_date(train_path)
    model = tf.keras.models.load_model(model_path)
    model.summary()

    print(y_test)
    for i in range(len(y_test)):
        xx = np.expand_dims(y_test[:,i],axis=0)
        yy =np.expand_dims(y_test[i],axis=0)
        test_loss, test_mse = model.evaluate(xx, yy,verbose=0)
        result = model.predict(xx)
        print("%d : test loss = %f, test mse = %f, prediction= [%f], TP = [%f]" % (i, test_loss, test_mse,result[0],y_test[i]))


if __name__ == "__main__":
    make_minute_model()
    train_path = 'bitcoin_minute.csv'
    model_path ='bitcoine_minute_%s_2.h5'%today
