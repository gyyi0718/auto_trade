...
- BE/트레일, 가변 타임아웃(ATR 기반), 쿨다운 유지
"""

# ===== imports =====
import os, time, math, csv, threading, logging
...
BE_EPS_BPS = 2.0
TRAIL_BPS = 50.0
TRAIL_AFTER_TIER = 2  # TP2 이후 트레일 시작
...
# universe
SYMBOLS = os.getenv("SYMBOLS","ASTERUSDT,AIAUSDT,0GUSDT,STBLUSDT,WLFIUSDT,LINEAUSDT,AVMTUSDT,BARDUSDT,SOMIUSDT").split(",")
...
# ===== compat loss stub (체크포인트에 기록됐을 가능성 대비) =====
...
def quiet_lightning_logs():
    """
    Lightning 내부 경고(특히 data_connector.py:433 반복 경고) 묵음 처리
    """
    ...
    for name in [
        ...
        "lightning.pytorch.trainer.connectors.logger_connector.logger_connector",
    ]:
...
def place_market(symbol: str, side: str, qty: float) -> bool:
    try:
        ...
        if q < rule["minOrderQty"]:
            LOGGER.warning(f"[place_market] too small qty -> {symbol} {side} req={qty}, rounded={q}, min={rule['minOrderQty']}")
            return False

        for attempt in range(3):
            q = _round_down(q, rule["qtyStep"])
            if q < rule["minOrderQty"]:
                LOGGER.warning(f"[place_market] below min after shrink -> {symbol} {side} q={q}")
                return False
            q_str = _fmt_qty(q, rule["qtyStep"])
            params = dict(
                category=CATEGORY, symbol=symbol, side=side,
                orderType="Market", qty=q_str, timeInForce="IOC",
                reduceOnly=False
            )
            LOGGER.info(f"[ORDER][MARKET][REQ] {symbol} {side} qty={q_str}")
            try:
                ret = _order_with_mode_retry(params, side=side)
                oid = ((ret or {}).get("result") or {}).get("orderId")
                LOGGER.info(f"[ORDER][MARKET][ACK] {symbol} {side} qty={q_str} oid={oid}")
                return True
            except Exception as e:
                msg = str(e)
                if (("110007" in msg) or ("10001" in msg)) and attempt < 2:
                    q *= 0.5
                    LOGGER.info(f"[ORDER][MARKET][RETRY] {symbol} shrink qty -> {q} (attempt {attempt+1}) reason={msg}")
                    continue
                raise

    except Exception as e:
        LOGGER.exception(f"[place_market][ERR] {symbol} {side} {qty} -> {e}")
        return False
...
def place_limit_postonly(symbol: str, side: str, qty: float, price: float) -> Optional[str]:
    try:
        ...
        if q < rule["minOrderQty"]:
            LOGGER.warning(f"[place_limit_postonly] too small qty -> {symbol} {side} req={qty}, rounded={q}, min={rule['minOrderQty']}")
            return None

        for attempt in range(3):
            q = _round_down(q, rule["qtyStep"])
            if q < rule["minOrderQty"]:
                LOGGER.warning(f"[place_limit_postonly] below min after shrink -> {symbol} {side} q={q}")
                return None
            q_str = _fmt_qty(q, rule["qtyStep"])
            px_str = _fmt_px(px, rule["tickSize"])
            params = dict(
                category=CATEGORY, symbol=symbol, side=side,
            ...
            try:
                ret = _order_with_mode_retry(params, side=side)
                oid = ((ret or {}).get("result") or {}).get("orderId")
                LOGGER.info(f"[ORDER][POSTONLY][ACK] {symbol} {side} qty={q_str} px={px_str} oid={oid}")
                return oid
            except Exception as e:
                msg = str(e)
                if (("110007" in msg) or ("10001" in msg)) and attempt < 2:
                    q *= 0.5
                    LOGGER.info(f"[ORDER][POSTONLY][RETRY] {symbol} shrink qty -> {q} (attempt {attempt+1}) reason={msg}")
                    continue
                raise
    except Exception as e:
        LOGGER.exception(f"[place_limit_postonly][ERR] {symbol} {side} {qty}@{price} -> {e}")
        return None
...
def place_reduce_limit(symbol: str, side: str, qty: float, price: float) -> bool:
    try:
        ...
        px = _round_to_tick(price, rule["tickSize"])
        if q < rule["minOrderQty"]:
            LOGGER.warning(f"[place_reduce_limit] too small qty -> {symbol} {side} req={qty}, rounded={q}, min={rule['minOrderQty']}")
            return False
...
def set_stop_with_retry(symbol: str, sl_price: Optional[float] = None, tp_price: Optional[float] = None, position_idx: Optional[int] = None) -> bool:
    ...
    try:
        ...
        LOGGER.exception(f"[TRADING_STOP][ERR] {symbol} -> {e}")
        return False
...
# 안전 allowlist (torch 2.6 weights_only=True 기본 대비)
add_safe_globals([GroupNormalizer, SignAwareNormalLoss])

# (중요) ckpt 로드시 커스텀 loss 잔재 무시 -> 기본 NormalDistributionLoss로 강제
MODEL = None
try:
...
def deepar_direction(symbol: str):
    """
    모델 신호 계산 + 단계별 로깅.
    Lightning 경고 스팸 방지를 위해 DataLoader num_workers=0 고정.
    """
    try:
        LOGGER.debug(f"[PRED] start symbol={symbol}")
        df = _recent_1m_df(symbol, minutes=SEQ_LEN+PRED_LEN+10)
        if df is None or len(df) < (SEQ_LEN+1):
            LOGGER.debug(f"[PRED][SKIP] {symbol} insufficient data len={0 if df is None else len(df)}")
            return None, 0.0
        ...
        side = None
        if   mu >  _THR: side = "Buy"
        elif mu < -_THR: side = "Sell"

        LOGGER.info(f"[PRED] {symbol} mu={mu:.6g} thr={_THR:.6g} -> {side}")
...
def try_enter(symbol: str, side_override: Optional[str] = None, size_pct: Optional[float] = None):
    ...
    if side not in ("Buy", "Sell") or conf < 0.55:
        LOGGER.debug(f"[SKIP][SIGNAL] {symbol} side={side} conf={conf:.2f}")
        return

    # 호가/ROI 필터
    bid, ask, mid = get_quote(symbol)
    if mid <= 0:
        LOGGER.debug(f"[SKIP][QUOTE] {symbol} mid<=0")
        return
    ...
    if qty <= 0:
        LOGGER.debug(f"[SKIP][QTY] {symbol} qty<=0 eq={eq:.2f} mid={mid:.4f}")
        return

    LOGGER.info(f"[ENTER][PREP] {symbol} side={side} conf={conf:.2f} mid={mid:.6f} qty={qty} edge={edge_bps:.1f} need>={need_bps:.1f}")

    # 동시 주문 구간
    with PLACE_LOCK:
        if len(get_positions()) >= MAX_OPEN:
            LOGGER.debug(f"[SKIP][SLOTS] positions>={MAX_OPEN}")
            return
        if COOLDOWN_UNTIL.get(symbol, 0.0) > time.time():
            LOGGER.debug(f"[SKIP][COOLDOWN] {symbol}")
            return
        if symbol in PENDING_MAKER or symbol in get_positions():
            LOGGER.debug(f"[SKIP][RACE] {symbol} already pending/position")
            return
        ...
        if EXECUTION_MODE == "maker":
            ...
            if oid:
                ...
                LOGGER.info(f"[ENTER][POSTONLY] {symbol} side={side} qty={qty} px={limit_px} oid={oid}")
            return
        ...
        LOGGER.info(f"[ENTER][TAKER][OK] {symbol} side={side} entry={entry:.6f} qty={qty} tp1={tp1:.6f} tp2={tp2:.6f} tp3={tp3:.6f} sl={sl:.6f} timeout_min={_min_hold}")
...
def _recent_vol_bps(symbol: str, minutes: int = None) -> float:
    ...
    lr = np.diff(np.log(np.asarray(closes)))
...
def _check_time_stop_and_cooldowns(symbol:str):
    ...
    if time.time() >= ddl:
        ...
        _order_with_mode_retry(dict(
            ...
            orderType="Market", qty=_fmt_qty(qty, get_instrument_rule(symbol)["qtyStep"]), reduceOnly=True, timeInForce="IOC"
        ), side=close_side)
...
# ====== DROP-IN: 대기중 PostOnly 관리 ======
def process_pending_postonly():
    now = time.time()
    if not PENDING_MAKER:
        return
...
def scanner_loop(iter_delay: float = 2.5):
    ...
    while True:
        try:
            ...
            if len(get_positions()) >= MAX_OPEN:
                LOGGER.debug("[SCANNER] slots full; sleep")
                time.sleep(iter_delay);
                continue
            ...
            LOGGER.info("[SCANNER] stop by user")
            break
        except Exception as e:
            LOGGER.exception(f"[SCANNER][ERR] {e}")
            time.sleep(2.0)
    ...
    while True:
        try:
            ...
            for sym in new_syms:
                ...
                LOGGER.info(f"[MONITOR][NEWPOS] {sym} {side} qty={qty} entry={entry:.6f} tp1={tp1:.6f} tp2={tp2:.6f} tp3={tp3:.6f} sl={sl:.6f} timeout_min={_min_hold}")
            ...
            for sym in closed_syms:
                ...
                LOGGER.info(f"[MONITOR][CLOSED] {sym} side={side} exit_mark={m:.6f}")
            ...
            LOGGER.info("[MONITOR] stop by user")
            break
        except Exception as e:
            LOGGER.exception(f"[MONITOR][ERR] {e}")
            time.sleep(2.0)
