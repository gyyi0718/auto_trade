# bybit_futures_hourly_fast.py
# -*- coding: utf-8 -*-
"""
Bybit USDT-Perp 1h(60분봉) collector — fast paging
"""
...
# =====================
SYMBOLS = [
    "EVAAUSDT", "1000PEPEUSDT", "AVNTUSDT", "BTRUSDT",  "COAIUSDT",
   ]
SYMBOLS = [
...
start_date = today - timedelta(days=365)  # 3년
START = start_date.strftime("%Y-%m-%d %H:%M:%S")
END = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")

SAVE_CSV = "bybit_futures_minutes_dash.csv"
SAVE_PARQUET = "bybit_futures_minutes_dash.parquet"
...
def fetch_ohlcv_descending(session, symbol, start_ms, end_ms, interval="60"):
    """
    내림차순 응답 특성에 맞춰 end 커서를 과거로 이동
    1시간봉: 각 캔들은 3600초(1시간) 간격
    """
    end_cursor = end_ms
    pages = []
    ...
    while end_cursor >= start_ms:
        ...
        # 1시간 = 3600초 = 3,600,000ms
        end_cursor = oldest_ts - 3_600_000
...
def main():
    ...
    with requests.Session() as s:
        s.headers.update({"User-Agent": "bybit-hourly-collector/1.0"})
